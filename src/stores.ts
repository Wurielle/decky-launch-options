...
function createStore<S>(
  state: S,
  options: StoreOptions<S, (state: S) => void> = {},
) {
...
const savedSettingsStoreState = settingsLocalStorageValue
  ? JSON.parse(settingsLocalStorageValue)
  : {}

export type LaunchOptionSort =
  | "local-alphabetical"
  | "local-active"
  | "global-alphabetical"
  | "global-active"
...
export const launchOptionSortOptions: Array<{
  data: LaunchOptionSort
  label: string
}> = [
...
export const settingsStore = createStore<SettingsStoreState>(
  {
    ...
    autoManageLaunchOptions:
      savedSettingsStoreState.autoManageLaunchOptions ??
      savedSettingsStoreState.autoUpdateLaunchOptions ??
      defaultSettingsStoreState.autoManageLaunchOptions,
  },
  {
    onUpdate() {
